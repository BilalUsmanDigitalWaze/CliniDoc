...
module.exports = {
  async getReceptionistHistroy( receptionistId) {
    ...
    const sql =`select Profile.*,Administrator.* ,Appointment.* ,Patient.* from Profile INNER JOIN Administrator INNER JOIN Appointment INNER JOIN Patient ON Profile.id=${receptionistId} and Administrator.profile_id=${receptionistId} and Appointment.administrator_id=Administrator.id and Appointment.patient_id=Patient.id`
